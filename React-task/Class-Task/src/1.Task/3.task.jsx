  ...
  return (
    ...
    <div style={{display:'flex', gap:'20px'}}>
      <div style={{display:'flex',justifyContent:'center',flexDirection:'column'}}>
        <h3>Open Tunnel</h3>
        <button onClick={openingTunnel} style={{backgroundColor:tunnelColor}}  >{openTunnel?"off":"on"}</button>
      </div>
      <div style={{display:'flex',justifyContent:'center',flexDirection:'column'}}>
        <h3>
            Enable power
        </h3>
        <button onClick={enablingPower} style={{backgroundColor:powerColor}} disabled={openTunnel}>{enablePower?"off":"on" }</button>
      </div>
      <div style={{display:'flex',justifyContent:'center',flexDirection:'column'}}>
        <h3>
            Start countdown
        </h3>
        <button onClick={startingCountdown} style={{backgroundColor:countDownColor}} disabled={enablePower} >{startCountdown?"off":"on"}</button>
      </div>
      <div style={{display:'flex',justifyContent:'center',flexDirection:'column'}}>
        <h3>
            Launch Missile 
        </h3>
        <button onClick={launchingMisslie} style={{backgroundColor:missileLaunchColor}} disabled={startCountdown}>{launchMisslie?"off":"on"}</button>
      </div>
    </div>
      <p>{successMassage}</p>
    </div>
